...
from pycalphad.core.sympydiff_utils import \
    build_functions as compiled_build_functions
...
def canonicalize(configuration, equivalent_sublattices):
    ...
    if equivalent_sublattices is not None:
        for subl in equivalent_sublattices:
            subgroup = sorted([configuration[idx] for idx in sorted(subl)],
...
def _symmetry_filter(x, config, symmetry):
    if x['mode'] == 'manual':
        ...
        for data_config in x['sublattice_configurations']:
            if canonicalize(config, symmetry) == canonicalize(data_config,
                                                              symmetry):
                return True
    return False
...
def _get_data(comps, phase_name, configuration, symmetry, datasets, prop):
    ...
    for idx, data in enumerate(desired_data):
        ...
        matching_configs = np.array([(canonicalize(sblconf,
                                                   symmetry) == canonicalize(
            configuration, symmetry))
                                     for sblconf in data['solver'][
                                         'sublattice_configurations']])
        matching_configs = \
        np.arange(len(data['solver']['sublattice_configurations']))[
            matching_configs]
        # Rewrite output values with filtered data
        desired_data[idx]['values'] = np.array(data['values'], dtype=np.float)[
            ..., matching_configs]
        desired_data[idx]['solver'][
            'sublattice_configurations'] = _list_to_tuple(
            np.array(data['solver']['sublattice_configurations'],
                     dtype=np.object)[matching_configs].tolist())
        try:
            desired_data[idx]['solver']['sublattice_occupancies'] = \
        ...
        desired_data[idx]['conditions']['T'] = \
        np.atleast_1d(data['conditions']['T'])[temp_filter]
        # Don't use data['values'] because we rewrote it above; not sure what 'data' references now
        desired_data[idx]['values'] = desired_data[idx]['values'][...,
                                      temp_filter, :]
    return desired_data
...
def _fit_parameters(feature_matrix, data_quantities, feature_tuple):
    ...
    for num_params in range(1, feature_matrix.shape[-1] + 1):
        ...
        rss = np.square(
            np.dot(current_matrix, clf.coef_) - data_quantities.astype(
                np.float)).sum()
...
def _get_samples(desired_data):
    ...
    for data in desired_data:
        ...
        num_configs = np.array(data['solver'].get('sublattice_configurations'),
                               dtype=np.object).shape[0]
        site_fractions = data['solver'].get('sublattice_occupancies',
                                            [[1]] * num_configs)
        site_fraction_product = [reduce(operator.mul, list(
            itertools.chain(*[np.atleast_1d(f) for f in fracs])), 1)
...
def _build_feature_matrix(prop, features, desired_data):
    ...
    feature_matrix = np.empty((len(all_samples), len(transformed_features)),
                              dtype=np.float)
    feature_matrix[:, :] = [
        transformed_features.subs({v.T: temp, 'YS': compf[0],
                                   'Z': compf[1]}).evalf()
...
def _shift_reference_state(desired_data, feature_transform, fixed_model):
    ...
    for dataset in desired_data:
        ...
        if dataset['solver'].get('sublattice_occupancies', None) is not None:
            value_idx = 0
            for occupancy, config in zip(
                    dataset['solver']['sublattice_occupancies'],
                    dataset['solver']['sublattice_configurations']):
                if dataset['output'].endswith('_FORM'):
                    pass
                elif dataset['output'].endswith('_MIX'):
                    values[..., value_idx] += feature_transform(
                        fixed_model.models['ref'])
                    pass
                else:
                    raise ValueError('Unknown property to shift: {}'.format(
                        dataset['output']))
                value_idx += 1
...
def sigfigs(x, n):
    if x != 0:
        return np.around(x, -(np.floor(np.log10(np.abs(x)))).astype(np.int) + (
        n - 1))
    else:
        return x
...
def fit_formation_energy(dbf, comps, phase_name, configuration, symmetry,
    ...
    if any([isinstance(conf, (list, tuple)) for conf in configuration]):
        fitting_steps = (
        ["CPM_FORM", "CPM_MIX"], ["SM_FORM", "SM_MIX"], ["HM_FORM", "HM_MIX"])
        ...
        print('ENDMEMBERS FROM INTERACTION: ' + str(
            _endmembers_from_interaction(configuration)))
    else:
    ...
    fixed_model = Model(dbf, comps, phase_name,
                        parameters={'GHSER' + c.upper(): 0 for c in comps})
    ...
    for num_sites, const in zip(dbf.phases[phase_name].sublattices,
        ...
        if 'VA' in const:
            ...
            1 - v.SiteFraction(phase_name, subl_idx, 'VA'))
        else:
            moles_per_formula_unit += num_sites
        subl_idx += 1

    for desired_props in fitting_steps:
        desired_data = _get_data(comps, phase_name, configuration, symmetry,
        ...
        if len(desired_data) > 0:
            ...
            feature_matrix = _build_feature_matrix(desired_props[0],
                                                   ...
                                                   desired_data)
            all_samples = _get_samples(desired_data)
            data_quantities = np.concatenate(
                _shift_reference_state(desired_data,
                                       feature_transforms[desired_props[0]],
                                       fixed_model),
                axis=-1)
            site_fractions = [_build_sitefractions(phase_name, ds['solver'][
                'sublattice_configurations'],
                                                   ds['solver'].get(
                                                           ...
                                                           ds['solver'][
                                                               'sublattice_configurations']),
                                                                len(ds[
                                                                        ...
                                                                        0])),
                                                               dtype=np.float)))
                              for ds in desired_data for _ in
                              ds['conditions']['T']]
            # Flatten list
            ...
            data_quantities = [
                sympy.S(i * moles_per_formula_unit).xreplace(sf).xreplace(
                    {v.T: ixx[0]}).evalf()
                for i, sf, ixx in
                zip(data_quantities, site_fractions, all_samples)]
            data_quantities = np.asarray(data_quantities, dtype=np.float)
            parameters.update(_fit_parameters(feature_matrix, data_quantities,
                                              features[desired_props[0]]))
            ...
            fixed_portion = np.dot(fixed_portion,
                                   [parameters[feature] for feature in
                                    features[desired_props[0]]])
...
def _build_sitefractions(phase_name, sublattice_configurations,
                         sublattice_occupancies):
    """
    ...
    for config, occ in zip(sublattice_configurations, sublattice_occupancies):
        sitefracs = {}
        config = [[c] if not isinstance(c, (list, tuple)) else c for c in
                  config]
        ...
        for sublattice_idx in range(len(config)):
            if isinstance(config[sublattice_idx], (list, tuple)) != isinstance(
                    occ[sublattice_idx], (list, tuple)):
                raise ValueError(
            ...
            if not isinstance(config[sublattice_idx], (list, tuple)):
                ...
                for comp, val in zip(config[sublattice_idx],
                                     occ[sublattice_idx]):
                    sitefracs[
                        v.SiteFraction(phase_name, sublattice_idx, comp)] = val
        result.append(sitefracs)
    return result


def _compare_data_to_parameters(dbf, comps, phase_name, desired_data, mod,
                                configuration, x, y):
    import matplotlib.pyplot as plt
    all_samples = np.array(_get_samples(desired_data), dtype=np.object)
    endpoints = _endmembers_from_interaction(configuration)
    interacting_subls = [c for c in _list_to_tuple(configuration) if
                         isinstance(c, tuple)]
    ...
    if len(endpoints) == 1:
        ...
        if temperatures.min() != temperatures.max():
            temperatures = np.linspace(temperatures.min(), temperatures.max(),
                                       num=100)
        else:
            # We only have one temperature: let's do a bar chart instead
            bar_chart = True
            temperatures = temperatures.min()
        endmember = _translate_endmember_to_array(endpoints[0], mod.ast.atoms(
            v.SiteFraction))[None, None]
        predicted_quantities = calculate(dbf, comps, [phase_name], output=yattr,
                                         T=temperatures, P=101325,
                                         points=endmember, model=mod,
                                         mode='numpy')
        if y == 'HM' and x == 'T':
        ...
        first_endpoint = _translate_endmember_to_array(endpoints[0],
                                                       mod.ast.atoms(
                                                           v.SiteFraction))
        second_endpoint = _translate_endmember_to_array(endpoints[1],
                                                        mod.ast.atoms(
                                                            v.SiteFraction))
        ...
        predicted_quantities = calculate(dbf, comps, [phase_name], output=yattr,
                                         T=300, P=101325, points=point_matrix,
                                         model=mod, mode='numpy')
        response_data = predicted_quantities[yattr].values.flatten()
        if not bar_chart:
            extra_kwargs = {}
            if len(response_data) < 10:
            ...
            fig.gca().set_xlabel(str(':'.join(endpoints[0])) + ' to ' + str(
                ':'.join(endpoints[1])))
            fig.gca().set_ylabel(plot_mapping.get(y, y))
        else:
            bar_labels.append('This work')
    ...
    for data in desired_data:
        ...
        if x == 'T' or x == 'P':
            ...
            if disordered_config:
                ...
                subl_idx = \
                np.nonzero([isinstance(c, (list, tuple)) for c in occ[0]])[0]
                if len(subl_idx) > 1:
                ...
                interactions = np.array([i[1][1] for i in _get_samples([data])],
                                        dtype=np.float)
            ...
            if y.endswith('_MIX') and data['output'].endswith('_FORM'):
                ...
                mod_latticeonly = Model(dbf, comps, phase_name,
                                        parameters={'GHSER' + c.upper(): 0 for c
                                                    in comps})
                mod_latticeonly.models = {key: value for key, value in
                                          mod_latticeonly.models.items()
                                          if key == 'ref'}
                temps = data['conditions'].get('T', 300)
                pressures = data['conditions'].get('P', 101325)
                points = _build_sitefractions(phase_name, data['solver'][
                    'sublattice_configurations'],
                                              data['solver'][
                                                  'sublattice_occupancies'])
                for point_idx in range(len(points)):
                    ...
                    points[point_idx].update(
                        {key: 0 for key in missing_variables})
                    # Change entry to a sorted array of site fractions
                    points[point_idx] = list(OrderedDict(
                        sorted(points[point_idx].items(), key=str)).values())
                points = np.array(points, dtype=np.float)
                # TODO: Real temperature support
                points = points[None, None]
                stability = calculate(dbf, comps, [phase_name],
        ...
        if not bar_chart:
            ...
            fig.gca().plot(indep_var_data, response_data,
    ...
    if bar_chart:
        ...
        fig.suptitle('{} (T = {} K)'.format(endmember_title, temperatures),
                     fontsize=20)
...
def plot_parameters(dbf, comps, phase_name, configuration, symmetry,
                    datasets=None):
    ...
    mod_norefstate = Model(dbf, comps, phase_name,
                           parameters={'GHSER' + c.upper(): 0 for c in comps})
    # Is this an interaction parameter or endmember?
    if any([isinstance(conf, list) or isinstance(conf, tuple) for conf in
            configuration]):
        plots = mix_plots
    else:
        plots = em_plots
    for x_val, y_val in plots:
        ...
        if y_val.endswith('_FORM'):
            _compare_data_to_parameters(dbf, comps, phase_name, desired_data,
                                        mod_norefstate, configuration, x_val,
                                        y_val)
        else:
            _compare_data_to_parameters(dbf, comps, phase_name, desired_data,
                                        mod, configuration, x_val, y_val)
...
def _list_to_tuple(x):
    def _tuplify(y):
        if isinstance(y, list) or isinstance(y, tuple):
            return tuple(
                _tuplify(i) if isinstance(i, (list, tuple)) else i for i in y)
        else:
            return y
...
def _generate_symmetric_group(configuration, symmetry):
    ...
    if symmetry is not None:
        while np.any(np.array(symmetry, dtype=np.object) != permute(
                permutation)):
...
def phase_fit(dbf, phase_name, symmetry, subl_model, site_ratios, datasets,
    ...
    print('{0} endmembers ({1} distinct by symmetry)'.format(all_em_count,
                                                             len(endmembers)))
    ...
    for endmember in endmembers:
        ...
        if ((len(endmember_comps) == 1) and (endmember_comps[0] != 'VA')) or \
                ((len(endmember_comps) == 2) and (endmember[-1] == 'VA') and (
                    len(set(endmember[:-1])) == 1)):
            ...
            if dbf.symbols.get(sym_name, None) is not None:
                num_moles = sum(
                    [sites for elem, sites in zip(endmember, site_ratios) if
                     elem != 'VA'])
    ...
    bin_interactions = sorted(
        set(canonicalize(i, symmetry) for i in transformed_bin_interactions),
    ...
    for interaction in bin_interactions:
        ...
        parameters = fit_formation_energy(dbf, sorted(dbf.elements), phase_name,
                                          ixx, symmetry, datasets)
                ...
                if key.has(check_symbol):
                    if value != 0:
                        ...
                        while dbf.symbols.get(symbol_name, None) is not None:
                            dbf.varcounter += 1
                            symbol_name = 'VV' + str(dbf.varcounter).zfill(4)
                        dbf.symbols[symbol_name] = sigfigs(parameters[key],
                                                           numdigits)
                        parameters[key] = sympy.Symbol(symbol_name)
        ...
        symmetric_interactions = _generate_symmetric_group(interaction,
                                                           symmetry)
...
def multi_plot(dbf, comps, phases, datasets, ax=None):
    ...
    for output, indep_var in plots:
        desired_data = datasets.search((tinydb.where('output') == output) &
                                       (tinydb.where('components').test(
                                           lambda x: set(x).issubset(comps))) &
                                       (tinydb.where('phases').test(
                                           lambda x: len(
                                               set(phases).intersection(
                                                   x)) > 0)))
        ...
        for data in desired_data:
            ...
            temp_repeats = np.zeros(
                len(np.atleast_1d(data['conditions'][indep_var])), dtype=np.int)
            for idx, x in enumerate(payload):
                temp_repeats[idx] = len(x)
            temps_ravelled = np.repeat(data['conditions'][indep_var],
                                       temp_repeats)
            payload_ravelled = []
            phases_ravelled = []
            comps_ravelled = []
            ...
            for rp in payload_ravelled:
                ...
                dependent_comp = list(
                    set(real_components) - set(comp_dict.keys()))
            ...
            for sym in symbols_ravelled:
                ...
                ax.scatter(comps_ravelled[selected], temps_ravelled[selected],
                           marker=sym, s=100,
                           c='none', edgecolors=[phase_color_map[x] for x in
                                                 phases_ravelled[selected]])
...
def _sublattice_model_to_variables(phase_name, subl_model):
    ...
    for idx, subl in enumerate(subl_model):
        result.extend(
            [v.SiteFraction(phase_name, idx, s) for s in sorted(subl, key=str)])
    return result


def estimate_hyperplane(dbf, comps, phases, current_statevars, comp_dicts,
    ...
    for cond_dict, phase_flag in comp_dicts:
        ...
        if np.any(np.isnan(list(cond_dict.values()))):
            ...
            multi_eqdata = equilibrium(dbf, comps, phases, cond_dict,
                                       verbose=False,
                                       model=phase_models,
            ...
            zero_dof = np.all((multi_eqdata['Y'].values == 1.) | np.isnan(
                multi_eqdata['Y'].values))
            if (num_phases == 1) and zero_dof:
                region_chemical_potentials.append(
                    np.full_like(np.squeeze(multi_eqdata['MU'].values), np.nan))
            else:
                region_chemical_potentials.append(
                    np.squeeze(multi_eqdata['MU'].values))
    # print('REGION_CHEMICAL_POTENTIALS', region_chemical_potentials)
    region_chemical_potentials = np.nanmean(region_chemical_potentials, axis=0,
                                            dtype=np.float)
    return region_chemical_potentials


def tieline_error(dbf, comps, current_phase, cond_dict,
                  region_chemical_potentials, phase_flag,
                  phase_models, parameters):
    ...
    if np.any(np.isnan(list(cond_dict.values()))):
        ...
        driving_force = np.multiply(region_chemical_potentials,
                                    single_eqdata['X'].values).sum(axis=-1) - \
                        single_eqdata['GM'].values
        desired_sitefracs = single_eqdata['Y'].values[...,
                            np.argmax(driving_force), :]
        ...
        num_dof = sum([len(set(c).intersection(comps)) for c in
                       dbf.phases[current_phase].constituents])
        ...
        for c in dbf.phases[current_phase].constituents:
            ...
            sitefracs_to_add[np.isnan(sitefracs_to_add)] = 1 - np.nansum(
                sitefracs_to_add)
        ...
        single_eqdata = equilibrium(dbf, comps, [current_phase], cond_dict,
                                    verbose=False,
                                    model=phase_models,
                                    scheduler=dask.async.get_sync,
                                    parameters=parameters)
        if np.all(np.isnan(single_eqdata['NP'].values)):
            ...
            print('Dumping', 'error-' + str(error_time) + '.py')
            with open('error-' + str(error_time) + '.py', 'w') as f:
                f.write(template_error.format(dbf.to_string(fmt='tdb'), comps,
                                              [current_phase], cond_dict,
                                              {key: float(x) for key, x in
                                               parameters.items()}))
        ...
        if np.all(np.isnan(single_eqdata['NP'].values)):
            ...
            return 0
        phases_idx = np.nonzero(
            ~np.isnan(np.squeeze(single_eqdata['NP'].values)))
...
def multi_phase_fit(dbf, comps, phases, datasets, phase_models,
    ...
    desired_data = datasets.search((tinydb.where('output') == 'ZPF') &
                                   (tinydb.where('components').test(
                                       lambda x: set(x).issubset(comps))) &
                                   (tinydb.where('phases').test(lambda x: len(
                                       set(phases).intersection(x)) > 0)))
    ...
    for data in desired_data:
        ...
        for region, region_eq in phase_regions.items():
            ...
            for req in region_eq:
                ...
                region_chemical_potentials = \
                    dask.delayed(estimate_hyperplane)(dbf, data_comps, phases,
                                                      current_statevars,
                                                      comp_dicts,
                                                      phase_models, parameters)
                ...
                for current_phase, cond_dict in zip(region, comp_dicts):
                    ...
                    for key, val in cond_dict.items():
                        if val is None:
                            cond_dict[key] = np.nan
                    cond_dict.update(current_statevars)
                    error = dask.delayed(tieline_error)(dbf, data_comps,
                                                        current_phase,
                                                        cond_dict,
                                                        region_chemical_potentials,
                                                        phase_flag,
...
def _multiphase_error(dbf, data, datasets, **kwargs):
    ...
    for phase_name in phases:
        ...
        desired_data = datasets.search(
            (tinydb.where('output').test(lambda k: k in desired_props)) &
            (
            tinydb.where('components').test(lambda k: set(k).issubset(comps))) &
            (tinydb.where('solver').test(
                lambda k: k.get('mode', None) == 'manual')) &
            (tinydb.where('phases') == [phase_name]))
        ...
        for idx, dd in enumerate(desired_data):
            temp_filter = np.nonzero(
                np.atleast_1d(dd['conditions']['T']) >= 298.15)
            # Necessary copy because mutation is weird
            desired_data[idx]['conditions'] = desired_data[idx][
                'conditions'].copy()
            desired_data[idx]['conditions']['T'] = \
            np.atleast_1d(dd['conditions']['T'])[temp_filter]
            # Don't use data['values'] because we rewrote it above; not sure what 'data' references now
            desired_data[idx]['values'] = np.asarray(
                desired_data[idx]['values'])[..., temp_filter, :]
        ...
        # Flatten list
        data_quantities = np.asarray(list(itertools.chain(*data_quantities)),
                                     dtype=np.object)
        site_fractions = [_build_sitefractions(phase_name, ds['solver'][
            'sublattice_configurations'],
                                               ds['solver'].get(
                                                   'sublattice_occupancies',
                                                   np.ones((len(ds['solver'][
                                                                    'sublattice_configurations']),
                                                            len(ds['solver'][
                                                                    'sublattice_configurations'][
                                                                    0])),
        ...
        for idx in range(len(site_fractions)):
            ...
            for subl_idx, subl_species in enumerate(
                    dbf.phases[phase_name].constituents):
                for spec in subl_species:
                    if v.SiteFraction(phase_name, subl_idx,
                                      spec) not in sf.keys():
                        sfsum = sum([val for key, val in sf.items() if
                                     key.sublattice_index == subl_idx])
                        sfsum = max(sfsum, 1e-16)
                        site_fractions[idx][
                            v.Y(phase_name, subl_idx, spec)] = 1 - sfsum
        # Remove existing partial model contributions from the data
        # print('DATA_QUANTITIES 1', data_quantities)
        data_quantities = data_quantities - np.repeat(
            [feature_transforms[ds['output']](fixed_model.GM)
             for ds in desired_data],
        ...
        data_quantities = [
            sympy.S(i).xreplace(sf).xreplace({v.T: ixx[0]}).evalf()
        ...
        multiply_array = [
            np.repeat(data_multipliers[ds['output'].split('_')[0]],
                      len(ds['values'].flat))
            for ds in desired_data]
        ...
        output_types = [np.repeat(ds['output'], len(ds['values'].flat)) for ds
                        in desired_data]
        output_types = list(itertools.chain(*output_types))
        dq[phase_name] = (
        data_quantities, site_fractions, all_samples, output_types)
    total_singlephase_dq = sum([len(x[0]) for x in dq.values()])

    error_vector = np.zeros((data_rows + total_singlephase_dq,), dtype=np.float)
    iter_idx = 0
    for x in dq.values():
        error_vector[data_rows + iter_idx:data_rows + iter_idx + len(x[0])] = x[
            0]
        iter_idx += len(x[0])
    data_idx = 0
...
def fit(input_fname, datasets, resume=None, scheduler=None, recfile=None,
    ...
    if resume is None:
        ...
        comp_refs = {c.upper(): stabledata[c.upper()] for c in dbf.elements if
                     c.upper() != 'VA'}
        comp_refs['VA'] = 0
        dbf.symbols.update(
            {'GHSER' + c.upper(): data for c, data in comp_refs.items()})
        for phase_name, phase_obj in sorted(data['phases'].items(),
            ...
            phase_fit(dbf, phase_name, symmetry, subl_model, site_ratios,
                      datasets, refdata, aliases=aliases)
    else:
    ...
    symbols_to_fit = sorted(
        [x for x in sorted(dbf.symbols.keys()) if pattern.match(x)])
    ...
    import pymc
    model_dof = [pymc.Normal(x, float(dbf.symbols[x]),
                             1. / (0.7407 * float(dbf.symbols[x])) ** 2,
                             value=float(dbf.symbols[x]))
                 for x in symbols_to_fit]
    ...
    error_args = ",".join(['{}=model_dof[{}]'.format(x, idx) for idx, x in
                           enumerate(symbols_to_fit)])
    error_code = """
    def error({0}):
    ...
    if recfile:
        recfile.write(','.join(
            ['error', 'time'] + [str(x) for x in symbols_to_fit]) + '\n')

    result_obj = {'model_dof': model_dof}
    error_context = {'data': data, 'comps': comps, 'dbf': dbf,
                     'phases': sorted(data['phases'].keys()),
    ...
    try:
        # pymc.MAP(pymod).fit()
        mdl.sample(iter=50000, burn=0, burn_till_tuned=False, thin=1,
                   progress_bar=True, save_interval=1)
    except:
        close_mdl()
    finally:
        close_mdl()
